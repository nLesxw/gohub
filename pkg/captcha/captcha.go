...
func NewCaptcha() *Captcha {
	once.Do(func() {
		...
		store := RedisStore {
			...
			KeyPrefix: config.GetStirng("app.name") + ":captcha:",
		}
		...
		// 实例化 bae64Captcha 并赋值给内部使用的 internalCaptcha 对象
...
// VerifyCaptcha 验证 验证码 是否正确
func (c *Captcha) VerifyCaptcha(id string, answer string) (match bool){

	// 方便本地和 API 自动测试
	if !app.IsProduction() && id == config.GetStirng("captcha.testing_key") {
		return true
	}
